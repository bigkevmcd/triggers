...
func TestHandleEventWithInterceptors(t *testing.T) {
	...
	req.Header.Add("X-Github-Delivery", "3e752b1d-85e3-4a71-b639-a9bbb0eefca0")
	req.Header.Add("X-Hub-Signature", test.GitHubSignature(t, testSecret, eventBody))
...
func TestExecuteInterceptor(t *testing.T) {
	...
	for _, method := range []string{
		...
		t.Run(method, func(t *testing.T) {
			...
			_, resp, header, err := r.executeInterceptors(trigger, req, []byte(`{}`), logger.Sugar())
			if err != nil {
...
func TestExecuteInterceptor_error(t *testing.T) {
	...
	if _, resp, _, err := s.executeInterceptors(trigger, req, nil, logger.Sugar()); err == nil {
...
func TestHandleEventWithInterceptorsAndTriggerAuth(t *testing.T) {
	for _, testCase := range []struct {
		...
		eventBody := []byte(`{"action": "opened", "head_commit": {"id": "testrevision"}, "repository": {"url": "testurl"}, "foo": "bar\t\r\nbaz昨"}`)
		...
		req.Header.Add("Content-Type", "application/json")
		req.Header.Add("X-Github-Event", "pull_request")
		req.Header.Add("X-Github-Delivery", "pull_request")
		req.Header.Add("X-Hub-Signature", test.GitHubSignature(t, testSecret, eventBody))
